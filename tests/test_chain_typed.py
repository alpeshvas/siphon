...
class TestChainSpec:
    ...
    def test_self_referential_then(self):
        ...
        spec = ChainSpec(from_="$.items[*]", then=ChainSpec(from_="subitems[*]"))
    ...
    def test_three_level_nesting(self):
        ...
        spec = ChainSpec(
            from_="$.level1[*]",
            then=ChainSpec(from_="level2[*]", then=ChainSpec(from_="level3[*]")),
        )
...
class TestChainSpecSerialization:
    ...
    def test_round_trip_serialization(self):
        ...
        original = ChainSpec(
            from_="$.items[*]", where={"status": "active"}, select={"id": "item_id"}, collect=True
        )
...
class TestQueryTyped:
    ...
    def test_simple_path(self, sample_data):
        ...
        spec = ChainSpec(from_="$.items[*]", where={"status": "active"}, collect=True)
    ...
    def test_with_select(self, sample_data):
        ...
        spec = ChainSpec(
            from_="$.items[*]", where={"status": "active"}, select={"item_id": "id"}, collect=True
        )
        result = query_typed(spec, sample_data)
        assert len(result) == 2
    ...
    def test_with_sort(self, sample_data):
        ...
        spec = ChainSpec(
            from_="$.items[*]", sort=[SortSpec(field="price", order="asc")], collect=True
        )
        result = query_typed(spec, sample_data)
    ...
    def test_with_limit(self, sample_data):
        ...
        spec = ChainSpec(from_="$.items[*]", limit=2, collect=True)
        result = query_typed(spec, sample_data)
        assert len(result) == 2
    ...
    def test_with_then(self):
        ...
        data = {
            "departments": [
                ...
                {"name": "Sales", "employees": [{"id": 3, "name": "Charlie"}]},
            ]
        }
        spec = ChainSpec(
            from_="$.departments[*]",
            select={"dept": "name"},
            then=ChainSpec(from_="employees[*]", select={"emp_name": "name"}, collect=True),
        )
